...
async def handle_media_stream(websocket: WebSocket,customer_number: str):
    ...
    initial_message = "Greet the user with 'Hello there! I am an AI voice assistant for BooKIA Hotel Group where comfort meets elegance.' repeat the message in French then in Arabic."
...
async def handle_media_stream(websocket: WebSocket ,customer_number: str):
    system_message = f"""
    You are a multilingual AI assistant specializing in collecting and storing customer feedback for the BooKIA Hotel Group. Your primary tasks include:
